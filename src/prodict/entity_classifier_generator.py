...
DEFAULT_CONFIG = PROJECT_ROOT / "data" / "/home/lestrada/projects/ProDICT/data/00_classifier_configs/configs_MWU/entity_model_config_CHDM.yaml"
...
# output directory
output_dir = PROJECT_ROOT / "data" / (conf.RUN_FOLDER_NAME or "output")
...
def class_specific_workflow(training_df, held_out_df, scaled_train, scaled_hold_out, peptides_df_binary, output_directory, tumor_type_name=TARGET_CLASS_NAME, mwu_filtering=True):
...
def feature_selection(target_z_scores_train_df, output_directory):
    ...
    search_n_splits = GRID_SEARCH_N_SPLITS or ELNET_N_SPLITS
    search_n_repeats = ELNET_N_REPEATS or 1

    grid_search_rows = []
    all_target_proteins = []
    cached_coefficients_paths = []
    grid_search_cache_dir = Path(output_directory) / f"{class_name}_grid_search_cache"

    try:
        if not FEATURE_SELECTION_L1_RATIOS or not FEATURE_SELECTION_C_VALUES:
            raise ValueError("FEATURE_SELECTION_L1_RATIOS and FEATURE_SELECTION_C_VALUES must be defined.")

        if grid_search_cache_dir.exists():
            shutil.rmtree(grid_search_cache_dir)
        grid_search_cache_dir.mkdir(parents=True, exist_ok=True)

        total_combinations = len(FEATURE_SELECTION_L1_RATIOS) * len(FEATURE_SELECTION_C_VALUES)
        print(f"Running wrapper-based grid search for {total_combinations} ElasticNet combinations...")

        combo_idx = 0
        for l1_ratio in FEATURE_SELECTION_L1_RATIOS:
            for C in FEATURE_SELECTION_C_VALUES:
                print("-" * 80)
                print(f"Testing l1_ratio={l1_ratio}, C={C}")

                try:
                    target_cross_val_coeffs = fs.elnet_wrapper(
                        target_z_scores_train_df,
                        classified_by=CLASSIFIED_BY,
                        tumor_type_name=f'{class_name}_grid_search',
                        l1_ratio=l1_ratio,
                        C=C,
                        output_directory=output_directory,
                        n_splits=search_n_splits,
                        n_repeats=search_n_repeats,
                        n_jobs=ELNET_N_JOBS,
                        export=False
                    )

                    cache_path = grid_search_cache_dir / f"grid_search_coefficients_{combo_idx:03d}.pkl"
                    target_cross_val_coeffs.to_pickle(cache_path)

                    target_stats, target_proteins = fs.statistic_from_coefficients(
                        target_cross_val_coeffs,
                        TARGET_CLASS,
                        output_directory,
                        export=False,
                        verbose=False
                    ...
                    target_mcc = pd.NA
                    if 'MCC_score' in target_stats.index and 'mean' in target_stats.columns:
                        target_mcc = target_stats.loc['MCC_score', 'mean']
                    protein_count = len(target_proteins) if target_proteins else 0
                    ...
                    grid_search_rows.append(
                        {
                            ...
                            'C': C,
                            'MCC': target_mcc,
                            'len(target_proteins)': protein_count
                        }
                    )
                    all_target_proteins.append(target_proteins if target_proteins else [])
                    cached_coefficients_paths.append(cache_path)

                    mcc_message = "NA" if pd.isna(target_mcc) else f"{float(target_mcc):.4f}"
                    print(f"MCC={mcc_message}")
                    print(f"Selected {protein_count} proteins for this combination")

                except Exception as combo_error:
                    print(f"Warning: grid search failed for l1_ratio={l1_ratio}, C={C}: {combo_error}")
                finally:
                    combo_idx += 1

        if not grid_search_rows:
            raise RuntimeError("No valid ElasticNet parameter combination completed successfully.")

        grid_search_df = pd.DataFrame(grid_search_rows)
        grid_search_df['MCC_sort'] = pd.to_numeric(grid_search_df['MCC'], errors='coerce').fillna(float('-inf'))
        ranked_grid_search_df = grid_search_df.sort_values(
            by=['MCC_sort', 'len(target_proteins)'],
            ascending=[False, True]
        )
        best_idx = ranked_grid_search_df.index[0]

        if len(ranked_grid_search_df) > 1:
            top_row = ranked_grid_search_df.iloc[0]
            second_row = ranked_grid_search_df.iloc[1]
            mcc_difference = top_row['MCC_sort'] - second_row['MCC_sort']

            if (
                top_row['MCC_sort'] > 0.5
                and mcc_difference <= 0.15
                and second_row['len(target_proteins)'] < top_row['len(target_proteins)']
            ):
                best_idx = ranked_grid_search_df.index[1]
                print(
                    "Selecting second-ranked row because MCC difference is within 0.15 "
                    "and it uses fewer proteins. This difference is based on the cost of one TP to FN in low sample positive class"
                )

        best_row = grid_search_df.loc[best_idx]
        best_target_proteins = all_target_proteins[best_idx]
        best_coefficients_cache_path = cached_coefficients_paths[best_idx]
        grid_results_path = os.path.join(output_directory, f'{class_name}_grid_search_results.xlsx')
        grid_search_df.drop(columns=['MCC_sort']).to_excel(grid_results_path, engine='xlsxwriter', index=True)

        print("-" * 80)
        print(f"Grid search results exported to: {grid_results_path}")
        print(f"Best grid search row index: {best_idx}")
        best_mcc_message = "NA" if pd.isna(best_row['MCC']) else f"{float(best_row['MCC']):.4f}"
        print(
            "Best parameters from wrapper-based grid search: "
            f"L1_ratio={best_row['L1_ratio']}, C={best_row['C']}, "
            f"MCC={best_mcc_message}, len(target_proteins)={int(best_row['len(target_proteins)'])}"
        )

        # Reuse the cached coefficients for the winning combination instead of rerunning ElasticNet.
        target_cross_val_coeffs = pd.read_pickle(best_coefficients_cache_path)
        target_cross_val_coeffs.to_excel(
            os.path.join(output_directory, f'{class_name}_features_coefficients.xlsx'),
            engine='xlsxwriter',
            index=False
        )
        print(
            f"DataFrame exported to: "
            f"{os.path.join(output_directory, f'{class_name}_features_coefficients.xlsx')}"
        )
        fs.statistic_from_coefficients(
            target_cross_val_coeffs,
            TARGET_CLASS,
            output_directory,
            export=True,
        ...
        shutil.rmtree(grid_search_cache_dir)
        print(f"Selected {len(best_target_proteins)} protein features")
        return best_target_proteins

    except Exception as e:
        print(f"Warning: Feature selection failed: {e}")
        if grid_search_cache_dir.exists():
            print(f"Grid search cache retained at: {grid_search_cache_dir}")
        return []
...
def main():
    ...
    target_training_df, target_ho_df, target_z_scores_train_df, target_z_scores_held_out_df = class_specific_workflow(
        training_df, held_out_df, z_scores_train_df, z_scores_held_out, peptides_df_binary, output_dir, tumor_type_name=TARGET_CLASS_NAME, mwu_filtering=False
    )
