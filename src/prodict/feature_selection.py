...
def _fit_elnet_fold(
    ...
    """Fit one ElasticNet fold and return coefficients plus evaluation metrics."""
    ...
    row = log_reg.coef_.ravel().tolist()
    row.append(log_reg.intercept_[0])

    y_predict_fold = log_reg.predict(x_test_fold)
    row.append(f1_score(y_test_fold, y_predict_fold, pos_label=1))
    row.append(f1_score(y_test_fold, y_predict_fold, pos_label=0))
    row.append(f1_score(y_test_fold, y_predict_fold, average='weighted'))
    row.append(matthews_corrcoef(y_test_fold, y_predict_fold))
...
def elnet_cross_val(
    ...
    """
    Return ElasticNet coefficients and fold-level scores across repeated stratified CV.

    Args:
        df: Input dataframe generated by the preprocessing pipeline.
        classified_by: Metadata column used for class labels.
        l1_ratio: ElasticNet L1 ratio.
        C: Inverse regularization strength.
        n_splits: Number of folds per repeat.
        n_repeats: Number of repeated CV rounds.
        random_state: Seed for RepeatedStratifiedKFold.
        n_jobs: Number of parallel fold jobs.
    """
    ...
    y_train = df['Classifier'].to_numpy()
    X_train_df = df.drop(columns=['Sample name', 'Classifier', classified_by, 'TCC'], axis=1, errors='ignore')
    X_train = X_train_df.to_numpy()

    repeated_skf = RepeatedStratifiedKFold(
        n_splits=n_splits,
        n_repeats=n_repeats,
        random_state=random_state
    )
    splits = list(repeated_skf.split(X_train, y_train))

    def build_fold_tasks():
        return (
            delayed(_fit_elnet_fold)(
                X_train,
                y_train,
                train_index,
                test_index,
    ...
    try:
        ...
        results = list(
            tqdm(
                parallel(build_fold_tasks()),
                total=len(splits),
                desc="ElasticNet folds",
                unit="fold"
            )
        )
    except TypeError:
    ...
    coeff_score_df = pd.DataFrame(results, columns=col_names)
...
def elnet_wrapper(
    ...
    An adaptation of bootstrapping
        ...
        For each fold:
            Split train/test
            Fit ElasticNet LogisticRegression
            Extract coefficients
            Predict test fold
            Calculate F1_1, F1_0, weighted F1, MCC
            ↓
        Concatenate all fold rows
            ↓
    ...
    print(
        f"Running ElasticNet with RepeatedStratifiedKFold: "
        f"{n_splits} splits x {n_repeats} repeats = {total_folds} fits"
    )

    df_concatenated = elnet_cross_val(
        df,
